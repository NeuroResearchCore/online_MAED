function model = MAEDRankingIncremental(candidates, selectNum,model,options)
%Reference:
%
...
%construct the kernel incrementally if the Euclid has been set up
%if exists a euclid from previous iteration, update incrementally
%otherwise calculate distance matrix from scratch
if(isfield(model,'Euclid'))
    [D,K] = constructKernelIncrementally(model,fea,options);
else
    [D,K] = constructKernel(model,fea,options);
end
...
%update the model
model.X = candidates.X(smpRank,:);
model.Y = candidates.Y(smpRank,:);
model.Euclid=D(smpRank,smpRank);
model.K=K(smpRank,smpRank);
end
